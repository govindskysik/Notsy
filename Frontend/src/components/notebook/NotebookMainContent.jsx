...
const NotebookMainContent = ({ notebook, topics = [], resources = [], loading, onAddTopic, onDeleteTopic }) => {
  const [deleting, setDeleting] = useState(null);

  const removeTopic = async (event, topicId) => {
    event.stopPropagation();
    if (deleting) return;
    setDeleting(topicId);
    try { await onDeleteTopic(topicId); } catch { toast.error("Failed to delete topic"); } finally { setDeleting(null); }
  };
